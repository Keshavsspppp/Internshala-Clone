...
const razorpay = new Razorpay({
  key_id: process.env.RAZORPAY_KEY_ID || "rzp_test_T3tbDnwaj6pkS2",
  key_secret: process.env.RAZORPAY_KEY_SECRET || "iz62TddVdKib8ZUp30E3WQl1"
});
...
<head>
  <meta charset="utf-8">
  <style>
    @import url('https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;600;700;850&display=swap');
    body {
...
<body>
  <div class="container">
    <div class="left-col">
      ${resumeData.photoUrl ? `<img class="avatar" src="${resumeData.photoUrl}" alt="Avatar">` : ''}
      
      <div class="left-title">Contact</div>
      <div class="contact-item"><strong>Email:</strong><br>${resumeData.email}</div>
      <div class="contact-item"><strong>Phone:</strong><br>${resumeData.phone}</div>
      <div class="contact-item"><strong>Location:</strong><br>${resumeData.location}</div>
      
      ${resumeData.personalInfo?.skills ? `
        <div class="left-title">Skills</div>
        <div style="margin-top: 10px;">
          ${resumeData.personalInfo.skills.split(',').map(s => `<span class="skill-tag">${s.trim()}</span>`).join('')}
        </div>
      ` : ''}
    ...
    <div class="right-col">
      <h1 class="name-title">${resumeData.name}</h1>
      ...
      ${resumeData.personalInfo?.about ? `
        ...
        <div class="summary-text">${resumeData.personalInfo.about}</div>
      ` : ''}
      ...
      ${resumeData.qualifications && resumeData.qualifications.length > 0 ? `
        ...
        ${resumeData.qualifications.map(edu => `
          <div style="margin-bottom: 15px;">
            ...
            <p class="item-desc" style="margin-bottom: 0;">Score: ${edu.percentage || edu.cgpa}</p>
          </div>
        `).join('')}
      ` : ''}
    </div>
...
router.post("/verify-payment", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const generatedSignature = crypto
      .createHmac("sha256", process.env.RAZORPAY_KEY_SECRET || "iz62TddVdKib8ZUp30E3WQl1")
    ...
    try {
      ...
      console.warn("Firebase upload failed, saving locally as fallback:", storageError.message);
      ...
      downloadUrl = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/resumes/${filename}`;
    }
